...
class BSRoformer20a(Fourier):
    def __init__(
        ...
        gamma = 4
        r=2
        self.encoder1 = TransformerLayers(dim=96*r, head_dim=96*r//n_heads, n_heads=n_heads, depth=2)
        self.encoder2 = TransformerLayers(dim=192*r, head_dim=192*r//n_heads, n_heads=n_heads, depth=2)
        self.encoder3 = TransformerLayers(dim=384*r, head_dim=384*r//n_heads, n_heads=n_heads, depth=2)
        self.encoder4 = TransformerLayers(dim=768*r, head_dim=768*r//n_heads, n_heads=n_heads, depth=2)

        self.fc_in = nn.Linear(out_channels * np.prod(self.patch_size), 96*r)
        self.encoder2_proj = nn.Linear(96*r * gamma, 192*r)
        self.encoder3_proj = nn.Linear(192*r * gamma, 384*r)
        self.encoder4_proj = nn.Linear(384*r * gamma, 768*r)

        #
        self.decoder1 = TransformerLayers(dim=768*r, head_dim=768*r//n_heads, n_heads=n_heads, depth=2)
        self.decoder2 = TransformerLayers(dim=384*r, head_dim=384*r//n_heads, n_heads=n_heads, depth=2)
        self.decoder3 = TransformerLayers(dim=192*r, head_dim=192*r//n_heads, n_heads=n_heads, depth=2)
        self.decoder4 = TransformerLayers(dim=96*r, head_dim=96*r//n_heads, n_heads=n_heads, depth=2)

        # self.decoder1_proj = nn.Linear(1536 // gamma, 768)
        self.decoder1_proj = nn.Linear(768*r, 768*r)
        self.decoder2_proj = nn.Linear(768*r // gamma, 384*r)
        self.decoder3_proj = nn.Linear(384*r // gamma, 192*r)
        self.decoder4_proj = nn.Linear(192*r // gamma, 96*r)
