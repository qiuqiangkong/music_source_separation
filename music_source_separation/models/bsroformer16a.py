...
class Patch(nn.Module):
    def __init__(self, patch_size: int, in_channels: int, out_channels: int):
        ...
        self.gru = nn.GRU(input_size=in_channels, hidden_size=64, num_layers=2, bias=True, batch_first=True, dropout=0., bidirectional=True)
        self.fc_in = nn.Linear(in_features=64 * np.prod(patch_size) * 2, out_features=out_channels)
        
        self.fc_out = nn.Linear(in_features=out_channels, out_features=64 * np.prod(patch_size))
        self.inv_gru = nn.GRU(input_size=64, hidden_size=32, num_layers=2, bias=True, batch_first=True, dropout=0., bidirectional=True)
        # self.fc_out2 = nn.Linear(64 * 2, )
    ...
    def patchify(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x, _ = self.gru(x)
        x = rearrange(x, '(b t1 f1) (t2 f2) d -> (b t1 f1) (t2 f2 d)', b=B, t1=T1, t2=T2, f1=F1, f2=F2)
        x = self.fc_in(x)
        x = rearrange(x, '(b t1 f1) d -> b d t1 f1', t1=T1, f1=F1)
