...
class BSRoformer(Fourier):
    def __init__(self, config: BSRoformerConfig) -> None:
        ...
        self.patch = Patch(
            ...
            in_channels=96 * np.prod(self.patch_size),
            out_channels=config.n_embd
        )

        self.fc1 = nn.Linear(64, 96)
        _config = TmpConfig(n_embd=96, n_head=8)
        self.t_blocks1 = nn.ModuleList(Block(_config) for _ in range(1))
        self.f_blocks1 = nn.ModuleList(Block(_config) for _ in range(1))
        self.t_blocks2 = nn.ModuleList(Block(_config) for _ in range(1))
        self.f_blocks2 = nn.ModuleList(Block(_config) for _ in range(1))
        ...
        t_rope1 = build_rope(seq_len=config.n_fft, head_dim=12)
        f_rope1 = build_rope(seq_len=self.fps * 20, head_dim=12)
        self.register_buffer(name="t_rope1", tensor=t_rope1)  # shape: (t, head_dim/2, 2)
        self.register_buffer(name="f_rope1", tensor=f_rope1)  # shape: (t, head_dim/2, 2)
    ...
    def forward(self, audio: torch.Tensor, target) -> torch.Tensor:
        ...
        #
        for t_block, f_block in zip(self.t_blocks1, self.f_blocks1):
            
            x = rearrange(x, 'b d t f -> (b f) t d')
            x = t_block(x, self.t_rope1, mask=None)  # shape: (b*f, t, d)

            x = rearrange(x, '(b f) t d -> (b t) f d', b=B)
            x = f_block(x, self.f_rope1, mask=None)  # shape: (b*t, f, d)
        ...
        #
        for t_block, f_block in zip(self.t_blocks2, self.f_blocks2):
