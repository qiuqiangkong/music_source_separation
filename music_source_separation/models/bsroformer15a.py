...
class Patch(nn.Module):
    def __init__(self, in_channels: int, out_channels: int):
        ...
        self.inv_conv1 = nn.Conv2d(out_channels, out_channels, kernel_size=(3, 3), padding=(1, 1))
        self.inv_conv2 = nn.Conv2d(out_channels, in_channels, kernel_size=(3, 3), padding=(1, 1))

        # self.inv_conv1 = nn.ConvTranspose2d(out_channels, out_channels, kernel_size=(3, 3), stride=(2, 2), padding=(0, 0))
        # self.inv_conv2 = nn.ConvTranspose2d(out_channels, out_channels, kernel_size=(3, 3), stride=(2, 2), padding=(0, 0))
        self.inv_bn1 = nn.BatchNorm2d(out_channels)
        self.inv_bn2 = nn.BatchNorm2d(out_channels)
    ...
    def patchify(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.conv1(F.leaky_relu_(self.bn1(x)))
    ...
    def unpatchify(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.inv_conv1(F.leaky_relu_(self.inv_bn1(x)))
        x = F.interpolate(x, scale_factor=(2, 2))
        x = self.inv_conv2(F.leaky_relu_(self.inv_bn2(x)))
