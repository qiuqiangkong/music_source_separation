...
class BSRoformerRoPE1D(Fourier):
    def __init__(self, config: BSRoformerConfig) -> None:

        super(BSRoformerRoPE1D, self).__init__(
            n_fft=config.n_fft, 
        ...
        # Build RoPE cache
        self.t_rope = RoPE(self.head_dim, max_len=2000)
        self.f_rope = RoPE(self.head_dim, max_len=2000)
    ...
    def forward(self, audio: torch.Tensor) -> torch.Tensor:
        ...
        for t_block, f_block in zip(self.t_blocks, self.f_blocks):
            ...
            x = t_block(x, rope=self.t_rope, pos=None, mask=None)  # shape: (b*f, t, d)

            x = rearrange(x, '(b f) t d -> (b t) f d', b=B)
            x = f_block(x, rope=self.f_rope, pos=None, mask=None)  # shape: (b*t, f, d)
