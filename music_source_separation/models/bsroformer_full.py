...
from music_source_separation.models.attention_rope2d import Block
# from music_source_separation.models.rope import build_rope
from music_source_separation.models.rope_2d import RoPE
...
class BSRoformerFull(Fourier):
    def __init__(self, config: BSRoformerConfig) -> None:

        super(BSRoformerFull, self).__init__(
            n_fft=config.n_fft, 
    ...
    def forward(self, audio: torch.Tensor) -> torch.Tensor:
        ...
        B, D, T, F_ = x.shape  # batch size

        pos_t = torch.arange(T).repeat_interleave(F_)  # (t*f,)
        pos_f = torch.arange(F_).repeat(T)  # (t*f,)
        pos = torch.stack((pos_t, pos_f), dim=-1)  # (t*f, 2)

        x = rearrange(x, 'b d t f -> b (t f) d')  # (b, t*f, d)

        for block in self.blocks:
            x = block(x, rope=self.rope, pos=pos, mask=None)

        x = rearrange(x, 'b (t f) d -> b d t f', t=T, f=F_)  # (b, t*f, d)
