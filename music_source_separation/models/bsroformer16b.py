...
class Patch(nn.Module):
    def __init__(self, patch_size: int, in_channels: int, out_channels: int):
        ...
        self.num_layers = 2
        self.gru = nn.GRU(input_size=in_channels * patch_size[1], hidden_size=out_channels, num_layers=self.num_layers, bias=True, batch_first=True, dropout=0., bidirectional=False)
        self.fc_in = nn.Linear(in_features=out_channels * self.num_layers, out_features=out_channels)
        
        self.fc_out = nn.Linear(in_features=out_channels, out_features=out_channels * self.num_layers)
        self.inv_gru = nn.GRU(input_size=1, hidden_size=out_channels, num_layers=self.num_layers, bias=True, batch_first=True, dropout=0., bidirectional=False)
        self.fc_out2 = nn.Linear(out_channels, in_channels * patch_size[1])
    ...
    def patchify(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = rearrange(x, 'b d (t1 t2) (f1 f2) -> (b t1 f1) t2 (f2 d)', t2=F2, f2=F2)
        x, h = self.gru(x)
        x = rearrange(h, 'h (b t1 f1) d -> (b t1 f1) (h d)', b=B, t1=T1, f1=F1)
        x = self.fc_in(x)
        x = rearrange(x, '(b t1 f1) d -> b d t1 f1', b=B, t1=T1, f1=F1)
    ...
    def unpatchify(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.fc_out(x)
        h = rearrange(x, '(b t1 f1) (h d) -> h (b t1 f1) d', b=B, t1=T1, f1=F1, h=self.num_layers)
        x = torch.zeros(h.shape[1], self.patch_size[0], 1).to(h.device)
        x, _ = self.inv_gru(x, h.contiguous())
        x = self.fc_out2(x)
        x = rearrange(x, '(b t1 f1) t2 (f2 d) -> b d (t1 t2) (f1 f2)', b=B, t1=T1, t2=T2, f1=F1, f2=F2)
