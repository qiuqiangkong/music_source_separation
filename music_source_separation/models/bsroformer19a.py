...
class BSRoformer(Fourier):
    def __init__(self, config: BSRoformerConfig) -> None:
        ...
        # Transformer blocks
        # self.t_blocks_enc1 = nn.ModuleList(Block(config) for _ in range(2))
        # self.f_blocks_enc1 = nn.ModuleList(Block(config) for _ in range(2))
        # self.t_blocks_enc2 = nn.ModuleList(Block(config) for _ in range(4))
        # self.f_blocks_enc2 = nn.ModuleList(Block(config) for _ in range(4))
        # self.t_blocks_enc3 = nn.ModuleList(Block(config) for _ in range(8))
        # self.f_blocks_enc3 = nn.ModuleList(Block(config) for _ in range(8))

        # self.t_blocks_dec1 = nn.ModuleList(Block(config) for _ in range(8))
        # self.f_blocks_dec1 = nn.ModuleList(Block(config) for _ in range(8))
        # self.t_blocks_dec2 = nn.ModuleList(Block(config) for _ in range(4))
        # self.f_blocks_dec2 = nn.ModuleList(Block(config) for _ in range(4))
        # self.t_blocks_dec3 = nn.ModuleList(Block(config) for _ in range(2))
        # self.f_blocks_dec3 = nn.ModuleList(Block(config) for _ in range(2))

        

        self.t_blocks_enc1 = nn.ModuleList(Block(config) for _ in range(2))
        self.f_blocks_enc1 = nn.ModuleList(Block(config) for _ in range(2))
        self.t_blocks_enc2 = nn.ModuleList(Block(config) for _ in range(2))
        self.f_blocks_enc2 = nn.ModuleList(Block(config) for _ in range(2))
        self.t_blocks_enc3 = nn.ModuleList(Block(config) for _ in range(2))
        self.f_blocks_enc3 = nn.ModuleList(Block(config) for _ in range(2))

        self.t_blocks_dec1 = nn.ModuleList(Block(config) for _ in range(2))
        self.f_blocks_dec1 = nn.ModuleList(Block(config) for _ in range(2))
        self.t_blocks_dec2 = nn.ModuleList(Block(config) for _ in range(2))
        self.f_blocks_dec2 = nn.ModuleList(Block(config) for _ in range(2))
        self.t_blocks_dec3 = nn.ModuleList(Block(config) for _ in range(2))
        self.f_blocks_dec3 = nn.ModuleList(Block(config) for _ in range(2))
    ...
    def forward(self, audio: torch.Tensor, target) -> torch.Tensor:
        ...
        # 1.4 Patchify
        x = self.patch1.patchify(x)  # shape: (b, d, t, f)

        # --- 2. Transformer along time and frequency axes ---
        for t_block, f_block in zip(self.t_blocks_enc1, self.f_blocks_enc1):
        ...
        #
        x1 = x
        x = self.patch2.patchify(x)

        for t_block, f_block in zip(self.t_blocks_enc2, self.f_blocks_enc2):
        ...
        #
        x2 = x
        x = self.patch3.patchify(x)

        for t_block, f_block in zip(self.t_blocks_enc3, self.f_blocks_enc3):
        ...
        # Decoder
        for t_block, f_block in zip(self.t_blocks_dec1, self.f_blocks_dec1):
        ...
        x = self.patch3.unpatchify(x)
        x = x + x2

        #
        for t_block, f_block in zip(self.t_blocks_dec2, self.f_blocks_dec2):
        ...
        x = self.patch2.unpatchify(x)
        x = x + x1

        #
        for t_block, f_block in zip(self.t_blocks_dec3, self.f_blocks_dec3):
