    ...
    def __getitem__(
        ...
        for stem in self.stems:
            ...
            for idx in index_dict[stem]:
                audio_names[stem].append(self.list_names[idx])
                path = str(Path(self.audios_dir, self.list_names[idx], "{}.wav".format(stem)))
        ...
        for stem in self.stems:
            ...
            # Load a clip
            for path, start_time, clip_duration in zip(audio_paths[stem], start_times[stem], clip_durations[stem]):
                audio = load(
                    path=path, 
                    sr=self.sr, 
                    offset=start_time, 
                ...
                if self.stem_transform is not None:
                    audio = self.stem_transform(audio)
