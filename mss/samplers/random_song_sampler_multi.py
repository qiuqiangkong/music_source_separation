...
class RandomSongSampler_multi:
    ...
    def get_indexes_dict(self) -> dict:
        ...
        if self.time_align == "strict":
            idxes = {name: self.random_permutation(N, mix_num) for name in ["song"]}

        elif self.time_align == "group":
            idxes = {name: self.random_permutation(N, mix_num) for name in ["target", "bg"]}

        elif self.time_align == "random":
            idxes = {name: self.random_permutation(N, mix_num) for name in self.stems}
    ...
    def random_permutation(self, n, mix_num) -> list:
        xs = []
