...
class APEBlock(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int) -> None:
        ...
        self.r1 = nn.Parameter(1e-5 * torch.ones(dim))
        self.r2 = nn.Parameter(1e-5 * torch.ones(dim))
    ...
    def forward(
        ...
        x = x + self.r1 * self.attn(self.norm1(x), rope, pos)
        x = x + self.r2 * self.ffn(self.norm2(x))
