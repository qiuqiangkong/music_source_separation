...
class BSRoformer24a(Fourier):
    def __init__(
        ...
        **kwargs
        ...
        self.bandsplit = BandSplit(
            ...
            n_fft=n_fft, 
            n_bands=n_bands,
            in_channels=audio_channels * 2 * 15,  # real + imag
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        complex_sp = self.fourier.encode(audio)  # (b, c, t, f, a)
        # self.fourier.decode(complex_sp, audio.shape[-1])

        x = torch.view_as_real(complex_sp)  # shape: (b, c, t, f, 2)
        x = rearrange(x, 'b c t f a k -> b (c a k) t f')
        ...
        x = rearrange(x, 'b (c a k) t f -> b c t f a k', c=audio.shape[1], k=2).contiguous()
...
class FractionalSTFT(nn.Module):
    def __init__(self, n_fft: int, hop_length: int, recompute=False):
        ...
        W_enc = W_new[0 : k2.shape[0]//2+1, :, :]

        
        # W_enc = rearrange(W_enc, 'k n a -> (k a) n')
        
        W_dec_path = "_W_dec_24a.pkl"
    ...
    def encode(self, x: Tensor) -> Tensor:
        ...
        x *= self.window
        # from IPython import embed; embed(using=False); os._exit(0)
        out = torch.einsum('bctn, nka -> bctka', x.to(torch.complex64), rearrange(self.W_enc, 'k n a -> n k a'))  # (b, c, t, k, a)
        return out
    ...
    def decode(self, x: Tensor, length: int | None) -> Tensor:
        ...
        # Inverse transform
        x_flip = torch.flip(x[..., 1 : -1, :], dims=[-2]).conj()
        x = torch.cat([x, x_flip], dim=-2)
        x = torch.einsum('bctfa, fna -> bctn', x, self.W_dec)
