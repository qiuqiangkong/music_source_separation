...
class Block(nn.Module):
    ...
    def forward(
        ...
        x = x + self.attn(self.norm1(x), rope, pos, axis, sizes)
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        if axis:
            q, k, v = self.split_axis(q, k, v, axis, sizes)
        ...
        if axis:
            x = self.combine_axis(x, axis, sizes)

        return x

    def split_axis(self, q, k, v, axis, sizes):
        if axis == "t":
            q = rearrange(q, 'b (t f) n h -> (b f) t n h', t=sizes[0])
            k = rearrange(k, 'b (t f) n h -> (b f) t n h', t=sizes[0])
            v = rearrange(v, 'b (t f) n h -> (b f) t n h', t=sizes[0])
        
        elif axis == "f":
            q = rearrange(q, 'b (t f) n h -> (b t) f n h', f=sizes[1])
            k = rearrange(k, 'b (t f) n h -> (b t) f n h', f=sizes[1])
            v = rearrange(v, 'b (t f) n h -> (b t) f n h', f=sizes[1])

        return q, k, v

    def combine_axis(self, x, axis, sizes):
        if axis == "t":
            x = rearrange(x, '(b f) t d -> b (t f) d', t=sizes[0], f=sizes[1])
        
        elif axis == "f":
            x = rearrange(x, '(b t) f d -> b (t f) d', t=sizes[0], f=sizes[1])
