...
class BSRoformerMagPhase3(Fourier):
    def __init__(
        self,
        audio_channels=2,
        sample_rate=48000,
        ...
        **kwargs
        ...
        self.bandsplit = BandSplit(
            ...
            n_fft=n_fft, 
            n_bands=n_bands,
            in_channels=audio_channels * 3,  # mag + cos + sin
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        mag = complex_sp.abs()  # (b, c, t, f)
        x = torch.view_as_real(complex_sp)  # shape: (b, c, t, f, 2)
        x = rearrange(x, 'b c t f k -> b (c k) t f')  # shape: (b, d, t, f)
        x = torch.cat((mag, x), dim=1)

        # mag = complex_sp.abs()  # (b, c, t, f)
        # phase = complex_sp.angle()  # (b, c, t, f), range: [-π, π)
        # tmp = mag * torch.exp(1.j * phase)

        # cos = torch.real(tmp)
        # sin = torch.imag(tmp)
        
        # x = torch.cat([mag, cos, sin], dim=1)
        T0 = x.shape[2]
        ...
        # mag, cos, sin = x.chunk(chunks=3, dim=1)
        # mag = mag.abs()
        # phase = torch.atan2(sin, cos)
        # mask = mag * torch.exp(1.j * phase)
        # from IPython import embed; embed(using=False); os._exit(0)
        mask_mag = F.leaky_relu(x[:, 0 : 2, :, :])
        tmp = rearrange(x[:, 2 :, :, :], 'b (c k) t f -> b c t f k', k=2).contiguous()
        mask_phase = torch.view_as_complex(tmp).angle()  # shape: (b, c, t, f)
        mask = mask_mag * torch.exp(1.j * mask_phase)
