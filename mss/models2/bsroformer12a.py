...
class FractionalSTFT(nn.Module):
    def __init__(self, n_fft: int, hop_length: int, n_bands: int):
        super().__init__()
        self.n_fft = n_fft
        ...
        # f = Tensor(librosa.mel_frequencies(n_mels=100, fmin=0, fmax=sr // 2))
        ...
        # axes[0].plot(W_enc[0].real)
        # axes[1].plot(W_enc[2].real)
        # axes[2].plot(W_enc[2000].real)
        # plt.savefig("_zz.pdf")
        # from IPython import embed; embed(using=False); os._exit(0)
        ...
        self.beta_dec = nn.Parameter(torch.ones(k2.shape[0]))
