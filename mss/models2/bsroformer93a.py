...
class BSRoformer93a(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        for n_fft in self.n_ffts:
            in_channels = audio_channels * n_fft * 2
            self.patches.append(Patch(in_channels, dim, (self.patch_size_t, 1)))
            self.unpatches.append(UnPatch(dim, in_channels, (self.patch_size_t, 1)))
        ...
        self.final = nn.Linear(len(self.n_ffts), 1)
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        for i in range(len(self.n_ffts)):
            complex_sp = self.stft(x, self.n_ffts[i], self.hop_length)  # (b, c, k, t, f)
            complex_sps.append(complex_sp)

        '''
        wavs = []
        for i in range(len(self.n_ffts)):
            x = self.istft(complex_sps[i], self.n_ffts[i], self.hop_length)
            wav = self.sb_filter.synthesis(x)
            wavs.append(wav)

        out = torch.stack(wavs, dim=0).mean(0)
        sdr = fast_sdr(audio.cpu().numpy(), out.cpu().numpy())
        ...
        # Patchify
        emb = 0
        for i in range(len(complex_sps)):
            B, C, K, T = complex_sps[i].shape[0 : 4]
            x = rearrange(torch.view_as_real(complex_sps[i]), 'b c k t f x -> b (c f x) t k')
        ...
        # Unpatchify
        for i in range(len(complex_sps)):
            x = self.unpatches[i](emb)
            x = x[:, :, 0 : T, :]
            x = rearrange(x, 'b (c f x) t k -> b c k t f x', c=C, x=2)
            mask = torch.view_as_complex(x.contiguous())
            sep_stft = complex_sps[i] * mask

            # Subband synthesis
            x = self.istft(sep_stft, self.n_ffts[i], self.hop_length)  # (b, c, k, t, f)
