...
from mss.models2.dsp.banks import mel_linear_banks2, erb_linear_banks
...
class SubbandFilter(nn.Module):
    ...
    def __init__(
        ...
        self.bandpass_filter_len = sr - 1
        self.upsample_filter_len = 1001
        ...
        # self.upsample = UpSample(self.factor, 201)
    ...
    def highpass(self, f: float) -> Tensor:
        h = firwin(
            numtaps=to_odd(self.bandpass_filter_len), 
            cutoff=f / (self.sr / 2), 
            pass_zero="highpass",
            window=self.window_type
        )
...
def upsample_modulate_sum(
    ...
    2. Modulate: Shift the baseband signal spectrum to ω0
...
if __name__ == '__main__':
    ...
    sb_filter = SubbandFilter(sr, banks, factor, chunk_size=16).to(device)
