...
class BandSplit(nn.Module):
    def __init__(
        ...
        nn.init.uniform_(self.pre_w, -bound, bound)
        ...
        total = sum([len(idxes) for idxes in nonzero_indexes])
    ...
    def transform(self, x: Tensor) -> Tensor:
        ...
        for q in range(self.Q):
            ...
            _y = torch.einsum('bitsw,iswo->btso', _x, _w) + _b  # (b, t, s, o)
            out[:, :, :, subbands] = rearrange(_y, 'b t s o -> b o t s')
    ...
    def inverse_transform(self, x: Tensor) -> Tensor:
        ...
        for q in range(self.Q):
            ...
            _y = torch.einsum('bots,oisw->btisw', _x, _w) + _b  # (b, t, i, s, w)
            _y = _y * melbanks * masks
            _y = rearrange(_y, 'b t i s w -> b i t s w')
            out.scatter_add_(dim=3, index=idxes.flatten()[None, None, None, :].repeat(B, self.in_channels, T, 1), src=_y.flatten(3))
