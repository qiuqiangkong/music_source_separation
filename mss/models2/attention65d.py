...
class APEBlock(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, n_bands: int) -> None:
        super().__init__()
        ...
        self.r1 = nn.Parameter(1e-5 * torch.ones(n_bands, dim))
        self.r2 = nn.Parameter(1e-5 * torch.ones(n_bands, dim))
    ...
    def forward(
        ...
        """
        # try:
        h = self.attn(self.norm1(x), rope, pos)
        h = rearrange(h, '(b f) t d -> b d t f', f=self.n_bands) * rearrange(self.r1, 'f d -> 1 d 1 f')
        x = x + rearrange(h, 'b d t f -> (b f) t d')

        h = self.ffn(self.norm2(x))
        h = rearrange(h, '(b f) t d -> b d t f', f=self.n_bands) * rearrange(self.r2, 'f d -> 1 d 1 f')
        x = x + rearrange(h, 'b d t f -> (b f) t d')
        # except:
