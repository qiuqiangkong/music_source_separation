...
class BandSplitFixMel(nn.Module):
    def __init__(
        self, 
        sr: float, 
        n_fft: int, 
    ...
    def init_melbanks(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        melbanks = librosa.util.fix_length(data=melbanks, size=melbanks.shape[-1]+r-1, axis=-1)
        melbanks = rearrange(melbanks, 'n (f1 f2) -> n f1 f2', f2=r)
        melbanks = np.mean(melbanks, axis=-1)
