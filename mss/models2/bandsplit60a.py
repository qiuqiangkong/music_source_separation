...
class BandSplit(nn.Module):
    def __init__(
        ...
        melbanks = melbanks[:, 0 : -1]
        ola_window = ola_window[0 : -1]
        ...
        self.pre_w = nn.Parameter(torch.zeros((in_channels, n_fft // 2, out_channels)))  # (i, f, o)
        self.post_w = nn.Parameter(torch.zeros((out_channels, in_channels, n_fft // 2)))  # (o, i, f)
        self.pre_b = nn.Parameter(torch.zeros((self.n_bands, out_channels)))  # (k, o)
        self.post_b = nn.Parameter(torch.zeros((in_channels, n_fft // 2)))  # (i, f)
    ...
    def inverse_transform(self, x: Tensor) -> Tensor:
        ...
        B, O_, T, F = x.shape
        I = self.in_channels
        out = torch.zeros((B, I, T, self.n_fft // 2), device=x.device)
        ...
        for q in range(self.Q):
            ...
            _b = self.post_b[:, subbands]  # (i, s)
            _y = torch.einsum('bots,oisw->bitsw', _x, _w) + _b[None, :, None, :, None]  # (b, i, t, s, w)
            _y = _y * melbanks
