...
from mss.models2.bases_haar import transform as bases_haar_transform
from mss.models2.bases_haar import inverse_transform as bases_haar_inverse_transform
from mss.models2.bases_haar import haar_matrix
...
class BSRoformer80b(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        #
        self.bases_haar = BasesHaar(n_window=2048, hop_length=480)
        self.patch_haar = Patch2(2, dim, (4, 32))
        self.unpatch_haar = UnPatch2(dim, 2, (4, 32))
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        # Wave
        x = self.bases_haar.encode(audio)
        x = self.pad_tensor2(x)  # x: (b, d, t, f)
        x_haar = self.patch_haar(x) 

        # import matplotlib.pyplot as plt
        # plt.matshow(x.abs().data.cpu().numpy()[0, 0].T, origin='lower', aspect='auto', cmap='jet')
        # plt.savefig("_zz.pdf")
        # from IPython import embed; embed(using=False); os._exit(0)
        
        # Concat
        x = torch.cat([x_2048, x_haar], dim=1)
        ...
        x = self.post_fc(x)
        x = rearrange(x, 'b t f d -> b d t f')
        x_2048, x_haar = x.chunk(chunks=2, dim=1)
        ...
        #
        x = self.unpatch_haar(x_haar, self.ac)
        x = x[:, :, 0 : T_2048, :]
        y_wave = self.bases_haar.decode(x, audio.shape[-1])
...
class BasesHaar(nn.Module):
    def __init__(self, n_window, hop_length):
        super().__init__()
        self.n_window = n_window
        self.hop_length = hop_length
        self.register_buffer(name="w", tensor=torch.from_numpy(haar_matrix(n_window)))
        self.register_buffer(name="window", tensor=torch.hann_window(n_window))

    def encode(self, x):
        B, C, L = x.shape
        x = rearrange(x, 'b c l -> (b c) l')
        x = bases_haar_transform(x, self.w, self.n_window, self.hop_length, self.window)
    ...
    def decode(self, x, length):
        ...
        x = bases_haar_inverse_transform(x, self.w, self.n_window, self.hop_length, self.window)
