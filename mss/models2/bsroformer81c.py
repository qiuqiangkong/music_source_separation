...
class BSRoformer81c(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        f_bnds = [0, W/8, W/4, W/2, W]

        # self.hop_lengths = [32, 32]
        # self.factors = [factor * 2, factor * 1]
        # f_bnds = [0, W/2, W]
        
        # Subband filter
        banks = mel_linear_banks(sr=sample_rate, n_bands=64, max_bandwidth=W)
        bandwidths = [bank[1] - bank[0] for bank in banks]
        indices = [[] for _ in range(len(f_bnds) - 1)]

        for i in range(len(banks)):
            for j in range(len(f_bnds) - 1):
                if f_bnds[j] <= bandwidths[i] <= f_bnds[j + 1]:
                    indices[j].append(i)
    ...
    def check_sdr(self, audio, complex_sp) -> None:
        ...
        y = self.sb_resampler.synthesis(y)
