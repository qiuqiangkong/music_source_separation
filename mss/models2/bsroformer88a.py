...
class BSRoformer88a(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.patch_size_t = 4
        max_bandwidth = 190
        factor = sample_rate // (max_bandwidth + 10)
        chunk_size = 16
        ...
        self.patch = Patch(in_channels, dim, (self.patch_size_t, 4))
        self.unpatch = UnPatch(dim, in_channels, (self.patch_size_t, 4))
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        # axes[1].plot(x[0, 0, 50, :].data.cpu().numpy())
        # axes[2].plot(x[0, 0, 100, :].data.cpu().numpy())
        # axes[3].plot(x[0, 0, 200, :].data.cpu().numpy())
        # plt.savefig("_zz.pdf")

        fig, axes = plt.subplots(4, 1, sharex=True)
        axes[0].matshow(complex_sp.abs()[0, 0, 0].T.data.cpu().numpy())
        axes[1].matshow(complex_sp.abs()[0, 0, 50].T.data.cpu().numpy())
        axes[2].matshow(complex_sp.abs()[0, 0, 100].T.data.cpu().numpy())
        axes[3].matshow(complex_sp.abs()[0, 0, 200].T.data.cpu().numpy())
        # axes[0].matshow(complex_sp.abs()[0, 0, 0].T.data.cpu().numpy())
        # axes[1].matshow(complex_sp.abs()[0, 0, 10].T.data.cpu().numpy())
        # axes[2].matshow(complex_sp.abs()[0, 0, 20].T.data.cpu().numpy())
        # axes[3].matshow(complex_sp.abs()[0, 0, 50].T.data.cpu().numpy())
        plt.savefig("_zz.pdf")
