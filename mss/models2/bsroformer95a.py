...
class BSRoformer95a(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # Patch
        in_channels = audio_channels * self.T2 * 2
        self.pre_fc = nn.Linear(in_channels, dim)
        self.post_fc = nn.Linear(dim, in_channels)
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        x0 = x
        x = rearrange(torch.view_as_real(x), 'b c k (t1 t2) x -> b t1 k (c t2 x)', t2=self.T2)
        x = self.pre_fc(x)
        x = rearrange(x, 'b t1 k d -> b d t1 k')
        B = x.shape[0]
        ...
        x = rearrange(x, 'b d t1 k -> b t1 k d')
        x = self.post_fc(x)
        x = rearrange(x, 'b t1 k (c t2 x) -> b c k (t1 t2) x', t2=self.T2, c=2)
