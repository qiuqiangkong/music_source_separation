...
class BSRoformer68a(Fourier):
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        x_wav = self.wav_encoder.encode(F.pad(audio, pad=(0, 4800)))  # (b, 384, 51x64)
        ...
        x = self.post_fc(x)
        x = rearrange(x, 'b t f d -> b d t f')
        x, x_wav = x.chunk(chunks=2, dim=1)  # (b, d, t, f)
...
class WavEncoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        ks = 129
        self.enc1 = nn.Conv1d(in_channels, out_channels, kernel_size=ks, stride=30, padding=(ks-1)//2)
        self.dec1 = nn.ConvTranspose1d(out_channels, in_channels, kernel_size=ks, stride=30, padding=(ks-1)//2)
